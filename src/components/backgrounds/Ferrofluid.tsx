...
export interface FerrofluidProps {
  ...
  colors?: string[];
  dpr?: number;
  flowDirection?: "down" | "left" | "right" | "up";
  fluidity?: number;
  glow?: number;
  mixBlendMode?: string;
...
const Ferrofluid: React.FC<FerrofluidProps> = ({
  ...
  const programRef = useRef<null | Program>(null);
  const meshRef = useRef<Mesh | null>(null);
  const geometryRef = useRef<null | Triangle>(null);
  ...
  useEffect(() => {
    ...
    const { arr, avg, count } = prepColors(colors);
